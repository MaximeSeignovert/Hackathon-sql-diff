...
pub fn render_diff_markdown(diff: &DiffResult) -> String {
    ...
    out.push("## Modifications de colonnes".to_owned());
...
pub fn render_diff_html(diff: &DiffResult) -> String {
    ...
    for table_diff in &diff.altered_tables {
        ...
        for col in &table_diff.removed_columns {
            lines.push_str(&format!(
                r#"<li class="py-1"><span class="inline-block min-w-6 rounded bg-rose-100 px-2 py-0.5 text-xs font-semibold text-rose-800">DROP</span> colonne <code class="rounded bg-slate-100 px-1.5 py-0.5 text-slate-800">{}</code> <span class="text-slate-500">({})</span></li>"#,
                escape_html(&col.name),
                escape_html(&col.data_type)
            ));
        }
        ...
        for fk in &table_diff.removed_foreign_keys {
            lines.push_str(&format!(
                r#"<li class="py-1"><span class="inline-block min-w-6 rounded bg-rose-100 px-2 py-0.5 text-xs font-semibold text-rose-800">DROP</span> fk <code class="rounded bg-slate-100 px-1.5 py-0.5 text-slate-800">{}</code> <span class="text-slate-500">({}) -&gt; {}({})</span></li>"#,
                escape_html(&fk.name),
                escape_html(&fk.columns.join(", ")),
                escape_html(&fk.referenced_table),
                escape_html(&fk.referenced_columns.join(", "))
